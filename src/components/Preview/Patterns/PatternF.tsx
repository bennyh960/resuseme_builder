...
const PatternX: React.FC = () => {
  ...
  return (
    <div className="bg-white text-gray-800">
      {/* Header Section */}
      <div className="flex flex-col items-center justify-between mb-4 space-y-2 sm:flex-row sm:space-y-0 sm:space-x-4">
        <div>
          <h1 className="text-2xl font-bold">{`${personalInfo.firstName} ${personalInfo.lastName}`}</h1>
          <p className="text-lg italic text-gray-600">{personalInfo.jobTitle}</p>
        </div>
      ...
      {summary && (
        <div className="mb-6">
          ...
          <div className="text-justify" dangerouslySetInnerHTML={{ __html: summary }}></div>
        </div>
      )}
        ...
        <div className="mb-6">
          ...
          {experiences.map((exp) => (
            <div key={exp.id} className="mb-4">
              ...
              <div className="text-justify" dangerouslySetInnerHTML={{ __html: exp.workSummary }}></div>
            </div>
          ))}
        </div>
      )}
        ...
        <div className="mb-6">
          ...
          {educations.map((edu) => (
            <div key={edu.id} className="mb-4">
              <h3 className="text-lg font-bold">{edu.schoolName}</h3>
              ...
              {edu.description && (
                <div className="text-justify" dangerouslySetInnerHTML={{ __html: edu.description }}></div>
              )}
            </div>
          ))}
        </div>
      ...
      <div className="mb-6">
        ...
        {additionalSections.certifications && (
          <div className="mb-4">
            ...
            <div className="text-justify" dangerouslySetInnerHTML={{ __html: additionalSections.certifications }}></div>
          </div>
        )}

        {additionalSections.custom.length > 0 && (
          <div className="mb-4">
            <h2 className="text-xl font-semibold border-b pb-1 mb-2">Custom Section</h2>
            <ul className="list-disc pl-5">
